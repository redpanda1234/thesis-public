using Plots
using Colors
using PGFPlotsX # Some bug w/ pgfplotsx() unable to save unless
                # this is loaded explicitly

function twist(θ; r=5, ω=2π^2, ϕ=0)
    if θ == 0
        return [0,0,0]
    else
        return [r * θ^3 * cos(ω/θ + ϕ), θ^2, θ^2]
    end
end
...
function plot_twists(save::Bool)
    ...
    if save
        ...
        plot(camera=(40, 40), size=(1500,1000), legend=false)
        plot!(xsa, ysa, zsa, color=:blue, lw=1)
        plot!(xsb, ysb, zsb, color=:red, lw=1)
        savefig("countable-3d-r2-perspective.pdf")


        plot(camera=(0, 0), size=(1500,1000), legend=false)
        plot!(xsa, ysa, zsa, color=:blue, lw=1)
        plot!(xsb, ysb, zsb, color=:red, lw=1)
        savefig("countable-3d-r2-side.pdf")


        plot(camera=(0, 90), size=(1500,1000), legend=false)
        plot!(xsa, ysa, zsa, color=:blue, lw=1)
        plot!(xsb, ysb, zsb, color=:red, lw=1)
        savefig("countable-3d-r2-top.pdf")
