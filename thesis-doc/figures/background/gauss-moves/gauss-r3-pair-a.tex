...
% \begin{figure}\centering
    ...
    \begin{tikzpicture}[scale=.6, every node/.style={scale=.6}]
      ...
      \node () at (-5,3) {\pgfuseimage{ra}};
      % \node () at (5,4) {\pgfuseimage{rb}};

      \def\myshift{5cm}
      \begin{scope}[xshift=-\myshift]
        \node (top) at (0,.5) {$(\cdots, i_u^+, j_u^+, \cdots, i_o^+,
          k_u^+, \cdots, j_o^+, k_o^+, \cdots)$};

        \node (bot) at (0,-.5) {$(\cdots, {\color{red} j_u^+},
          {\color{red} i_u^+}, \cdots, {\color{red}k_u^+},
          {\color{red}i_o^+}, \cdots, {\color{red} k_o^+}, {\color{red}
            j_o^+}, \cdots)$};
