...
\begin{tikzpicture}[tdplot_main_coords]% [x={(0.707cm,0.707cm)},z={(0cm,1cm)},y={(-0.866cm,0.5cm)}]
  ...
  \tikzset{declare function={myx(\z)=sqrt(abs(\z))*cos(\z*180);}}
  \tikzset{declare function={myy(\z)=sqrt(abs(\z))*sin(\z*180);}}
  \tikzset{declare
    function={myz(\z)=1.5*sqrt(abs(\z/7))*sin(\z*342)*cos(200*\z-2) + \zshift;}}

  \pgfmathsetmacro{\zmin}{0}
  \pgfmathsetmacro{\znext}{\zmin+.01}
  \pgfmathsetmacro{\zmax}{3}


  \draw[densely dashed, very thin] ({myx(0)},{myy(0)},{0})
  \foreach \z in {\zmin,\znext,...,\zmax}{
    -- ({myx(\z)},{myy(\z)},{0})
  };


  \pgfmathsetmacro{\shadegap}{.005}
  \pgfmathsetmacro{\zshadenext}{\zmin+\shadegap}
  \pgfmathsetmacro{\zend}{\zmax-\shadegap}
  \foreach \z in {\zmin,\zshadenext,...,\zend}{
