\documentclass[border=1pt]{standalone}
% \usepackage[margin=15mm,a3paper,landscape]{geometry}
\usepackage{tikz}
\usepackage{tikz-3dplot}


\begin{document}
\tdplotsetmaincoords{0}{120}
\begin{tikzpicture}[tdplot_main_coords]% [x={(0.707cm,0.707cm)},z={(0cm,1cm)},y={(-0.866cm,0.5cm)}]

  \pgfmathsetmacro{\zshift}{-3}
  \tikzset{declare function={myx(\z)=sqrt(\z)*cos(\z*180);}}
  \tikzset{declare function={myy(\z)=sqrt(\z)*sin(\z*180);}}
  \tikzset{declare
    function={myz(\z)=1.5*sqrt(\z/7)*sin(\z*342)*cos(200*\z-2) + \zshift;}}

  \pgfmathsetmacro{\zmax}{3}


  \node[fill=black, circle, inner sep=1pt] (s0) at ({myx(0)}, {myy(0)}, {myz(0)}) {};
  \node[fill=black, circle, inner sep=1pt] (t0) at ({myx(\zmax)}, {myy(\zmax)}, {myz(\zmax)}) {};
  \begin{scope}[every node/.style={scale=.75}]
    \node[above left] () at (s0) {$s_0$};
    \node[above left] () at (t0) {$t_0$};
  \end{scope}


  \draw[] ({myx(0)}, {myy(0)}, {myz(0)})
  \foreach \z in {0,0.01,...,\zmax}{
    -- ({myx(\z)},{myy(\z)},{myz(\z)})
  };

  % \pgfmathsetmacro{\shadegap}{.005}
  % \pgfmathsetmacro{\zend}{\zmax-\shadegap}
  % \foreach \z in {0,\shadegap,...,\zend}{
  %   \filldraw[blue!30!white,opacity=0.3]
  %   ({myx(\z)},{myy(\z)},0) --
  %   ({myx(\z)},{myy(\z)},{myz(\z)}) --
  %   ({myx(\z+\shadegap)},{myy(\z+\shadegap)},{myz(\z+\shadegap)}) --
  %   ({myx(\z+\shadegap)},{myy(\z+\shadegap)},0) -- cycle;
  %   % \fill[gray, opacity=.1]
  %   % ({myx(\z)}, {myy(\z)}, -2) --
  %   % ({myx(\z)}, {myy(\z)}, 2) --
  %   % ({myx(\z+\shadegap)}, {myy(\z+\shadegap)}, 2) --
  % }



  % \draw[densely dashed, very thin] ({myx(0)},{myy(0)},{0})
  % \foreach \z in {0,0.01,...,\zmax}{
  %   -- ({myx(\z)},{myy(\z)},{0})
  % };

  % \filldraw[opacity=.2, gray] (-2,-2,0) -- ++(4,0,0) -- ++(0,4,0) --
  % ++(-4,0,0) -- cycle


  % \node[rotate=90,right=1cm] at (0,0,12) {to floor, gray};
\end{tikzpicture}

\end{document}
