...
ANCHOR_NAMES = dict(left="start", center="middle", right="end")
...
class SvgTurtle(TNavigator, TPen):
    ...
    def _goto(self, end):
        if self.screen:
            ...
            if self._lines_to_draw is not None:
                self._lines_to_draw.append((x1, y1, x2, y2, pencolor, pensize))
        self._position = end
    ...
    def fill(self, flag=None):
        ...
        if self._lines_to_draw:  # TODO: and len(self._path) > 2:
            ...
            self.screen.cv.add(
                self.screen.cv.polygon(
                    points=points, fill=self._fillcolor, fill_rule="evenodd"
                )
            )
        self._flush_lines()
        if not flag:
