...
# If you've written out the cycle representations to a separate file (see
# gen_tables()), then you can load it here to avoid recomputing some things.
...
def reduced_orbit(ind):
    ...
    for i in range(1, kp.order()):
        l = len(r1_r2_reduced_action(kp ** i))
        if l not in orblens:
            orblens[l] = [i]
        else:
            orblens[l] += [i]
